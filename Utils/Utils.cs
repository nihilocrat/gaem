...
public class Utils
{
	...
	public static object CallMethod(object src, string methodName, object arg)
	{
		return CallMethod(src, methodName, new object[]{ arg });
	}
	
	public static object CallMethod(object src, string methodName, object[] args)
	{
		System.Reflection.MethodInfo i = src.GetType().GetMethod(methodName);
		
		if(i != null)
		{
			return i.Invoke(src, args);
		}
		else
		{
    ...
    public static void CopySharedFields(object src, object dest)
    {
        ...
        for(int i=0; i < fields.Length; i++)
        {
            ...
            System.Reflection.FieldInfo destF = destType.GetField(name);
            if (destF != null)
            {
                destF.SetValue(dest, value);
            }
            else
            {
                Debug.LogErrorFormat("When copying types '{0} -> {1}', didn't find expected field '{2}'", srcType, destType, name);
            }
        }
    }
    ...
    {
        return GetLocalAngleTowards(sourceTransform, targetPosition, Vector3.up);
    }

	static public float GetLocalAngleTowards(Transform sourceTransform, Vector3 targetPosition, Vector3 up)
	{
		...
		var lookAngle = Vector3.Angle(up, localVec2Target);
    ...
    static public Bounds GetCombinedBounds(GameObject obj)
    {
    ...
    static public void ScaleToBounds(Transform host, Vector3 maxBounds)
    {
        ScaleToBounds(host, maxBounds, Vector3.zero);
    }

    static public void ScaleToBounds(Transform host, Vector3 maxBounds, Vector3 minBounds)
	{
        ...
        Vector3 gfxSize = GetCombinedBounds(host.gameObject).size;
        ...
        // check min bounds now
        if(diff == 0f && minBounds != Vector3.zero)
        {
            diffVec = gfxSize - minBounds;

            if (diffVec.x < 0f)
            {
                diff = minBounds.x / gfxSize.x; //diffVec.x;
            }
            else if (diffVec.y < 0f)
            {
                diff = minBounds.y / gfxSize.y; //diffVec.y; 
            }
            else if (diffVec.z < 0f)
            {
                diff = minBounds.z / gfxSize.z; //diffVec.z; 
            }
        }

		if(diff != 0f)
		{
			host.localScale = Vector3.one * diff;// + (Vector3.one * diff);
		}
	}
...
def decToRoman(num,s,decs,romans):
	...
	*/
    ...
    static public void SetUVs(Mesh mesh, Vector2 offset)
    {
        var newUVs = new Vector2[mesh.uv.Length];
    ...
    public static void CombineMeshes(Transform transform)
    {
        /*
        ...
        */
        ...
        for (var i = 0; i < meshFilters.Length; i++)
        {
            ...
            meshFilters[i].gameObject.SetActive(false);//GetComponent<MeshRenderer>().enabled = false;
        }
        ...
        render.sharedMaterial = meshFilters[0].GetComponent<MeshRenderer>().sharedMaterial;
    ...
    public static Texture2D LoadTextureFromFile(Texture2D baseTexture, string savedImageName)
    {
        Texture2D tex = baseTexture.DeepClone<Texture2D>();
    ...
    // for copying lists on AOT-only platforms
    public static List<T> CopyList<T>(T[] array)
    {
    ...
    {
        return CopyList<T>(list.ToArray());
    }
}
